...
while active:
    ...
    if login in logins:
        ...
        while db_active:
            ...
            my_lap = DriverProfile(track, lap_time, wheel_hardware)
            my_lap.record_lap_time(track, lap_time) #storing into db data [defined my key to which I will assign values =]
            print(f"Data we received so far: {my_lap.record_lap_time(track, lap_time)}") #checking is it works
            repeat = input("Would you like to let more data (yes/no)")
            if repeat == 'no':
                db_active = False  #stoping the loop
                print("|||||RESULT|||||")
                for name, lap in my_lap.record_lap_time(track, lap_time): # I missed this step trying to make trio nam3,ag3,prog_lang instead of pair
                    print(f"track: {name}:\ntime: {lap[0]}")
    else:
        name = input("I dont see your data\nLet me know your name:\n")
        primary_game = input("Let me know your primary game:\n")
        wheel_hardware = input("What wheel hardware you use?(moza, logitech, fnatec..)\n")
        my_driver = F1Driver(track, lap_time)
